"""Quick end-to-end verification on a 5s MyVideo_1 clip with latest settings."""
...
async def main():
    ...
    print(f"[TEST5S] Avatar ready, submitting job: {TEXT!r}")
    job_id = await pipeline.submit(upload_id, TEXT)
    print(f"[TEST5S] job_id={job_id}")
    ...
    for _ in range(600):
        ...
        print(
            f"[TEST5S] {time.strftime('%H:%M:%S')} status={st['status']} "
    ...
    print(f"[TEST5S] Output saved to {output_path}")
    ...
    print(f"[TEST5S] sharpness={sharp:.2f} frames={frames}")
    return output_path


if __name__ == "__main__":
    out = asyncio.run(main())
    print("[TEST5S] final output:", out)
