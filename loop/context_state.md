...
### 刚完成
- Phase 9 嘴部清晰度优化代码实现并验证：默认 avatar A/B 测试、sample/MyVideo_1.mp4 全流程 API 验证。
- 后端容器已重启（`docker compose restart backend`），新代码已生效。
- 仓库已通过 SSH 推送到 GitHub：`bash scripts/push_github_ssh.sh` 成功，`main` 分支更新。

### 当前未完成任务
- 无。

### 已知坑（新增/确认）
- 修改 `THG_BLUR_RATIO` 等 mask 参数后，需要删除 `output/v15/avatars/<upload_id>` 缓存后重新预处理。
- 长视频（如 26s/658 帧 720p）首次 MuseTalk 预处理可能需要 30-40 分钟；后续同 video 复用缓存则快很多。
